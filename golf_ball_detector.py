...
class GolfBallDetector:
    def __init__(self):
        ...
        self.detection_params = {
            "hsv_lower": np.array([160, 100, 50]),
            "hsv_upper": np.array([180, 255, 100]),
            "circularity_threshold": 0.85,  # Further relaxed for lower light conditions
            "hough_param1": 30,  # Reduced for better detection in lower light
            "hough_param2": 12,  # Reduced for more sensitivity in lower light
    ...
    def start_camera(self):
        ...
        preview_config = self.picam2.create_video_configuration(
            ...
            controls={
                "FrameRate": 60.0,  # Request 120 FPS
        ...
        self.picam2.start()
        print(
            "Camera started with resolution 320x240 at target 120 FPS (optimized for lower light)"
        )
    ...
    def detect_golf_ball(self, frame):
        ...
        mask = cv2.inRange(
            hsv, self.detection_params["hsv_lower"], self.detection_params["hsv_upper"]
        )
        ...
        for contour in contours:
            ...
            if circularity > self.detection_params["circularity_threshold"]:
                ...
                for existing_center, existing_radius, _, _ in detected_circles:
                    ...
                    distance = abs(center[0] - existing_center[0]) + abs(
        ...
        if (
            len(self.previous_detections)
            > self.detection_params["consistency_threshold"]
        ):
            self.previous_detections.pop(0)

        # Simplified temporal consistency logic for performance
        if (
            self.detection_params["temporal_consistency"]
            and len(self.previous_detections) >= 2
        ):  # Reduced from 3 to 2
            ...
            for center, radius, method, score in detected_circles:
                ...
                for prev_frame in recent_frames:
                    for prev in prev_frame:
                        # Manhattan distance is faster than Euclidean
                        if (
                            abs(prev[0] - detection[0]) < position_tolerance
                            and abs(prev[1] - detection[1]) < position_tolerance
                        ):
                            appearances += 1
                            break
    ...
    def create_display_frame(self, original_frame, circles, mask, edges):
        ...
        for center, radius, method, score in circles:
            ...
            color = (
                (0, 255, 0) if method == "hough" else (0, 165, 255)
            )  # Green for Hough, Orange for contour
        ...
        display_frame[5 : 5 + mask_small.shape[0], 5 : 5 + mask_small.shape[1]] = (
            mask_color
        )
        ...
        display_frame[
            5 : 5 + edges_small.shape[0], w - 5 - edges_small.shape[1] : w - 5
        ] = edges_color
        ...
        cv2.putText(
            display_frame,
            "EDGES",
            (w - 5 - edges_small.shape[1], 5 + edges_small.shape[0] + 10),
            cv2.FONT_HERSHEY_SIMPLEX,
            0.3,
            (255, 255, 255),
            1,
        ...
        curr_time = time.time()
        processing_fps = (
            1 / (curr_time - self.prev_time) if hasattr(self, "prev_time") else 0
        )
        ...
        try:
            while self.running:
                ...
                if frame_count % 500 == 0:  # Less frequent reporting at high FPS
                    ...
                    print(
                        f"Processed {frame_count} frames at {frame_count/elapsed:.2f} FPS"
                    )
